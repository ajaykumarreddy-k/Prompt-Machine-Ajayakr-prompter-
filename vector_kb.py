...
KB_MD_DIR     = Path("knowledge-base")          # your .md component files
INDEX_DIR     = Path("vector-kb")
CHUNK_SIZE    = 600
CHUNK_OVERLAP = 80
TOP_K         = 6
...
def build():
    ...
    # 3. Extra RAG Bundles (.pkl)
    EXTRA_BUNDLE = Path("ui/uxskill-smallrag/uupm_rag_bundle.pkl")
    if EXTRA_BUNDLE.exists():
        print(f"\n── Loading extra RAG bundle: {EXTRA_BUNDLE.name}")
        try:
            with open(EXTRA_BUNDLE, "rb") as f:
                ...
                for i, item in enumerate(meta):
                    bundle_chunks.append({
                        ...
                        "source":   item.get("source", "uupm_bundle"),
                        "category": "design",
                        "tag":      tag_chunk(item.get("content", "")),
                    })
                all_chunks.extend(bundle_chunks)
                print(f"  Handled {len(bundle_chunks)} reasoning/design rules from bundle.")
        except Exception as e:
            print(f"  ⚠  Failed to load extra bundle: {e}")
