...
{{
  ...
  "font_pair":        "Inter|Geist|Plus Jakarta Sans"
    ...
    VALID_EFFECTS = {"corner-blur", "gradient-mesh", "none"}
    VALID_PLACEMENTS = {"top-right", "top-left", "bottom-right", "bottom-left"}
    VALID_ANIMATIONS = {"typewriter", "fade-in", "slide-up", "gradient-text", "none"}

    visual_effect = extras.get("visual_effect", "")
    if visual_effect not in VALID_EFFECTS:
        visual_effect = "corner-blur" if p["dark"] else "none"

    effect_placement = extras.get("effect_placement", "")
    if effect_placement not in VALID_PLACEMENTS:
        effect_placement = "top-right"

    text_animation = extras.get("text_animation", "")
    if text_animation not in VALID_ANIMATIONS:
        text_animation = "fade-in"
    border_radius    = extras.get("border_radius",     "rounded-lg")
