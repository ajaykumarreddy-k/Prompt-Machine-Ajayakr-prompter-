...
# ── PALETTES (your existing P-1 to P-5) ──────────────────────────────────────
...
def _rule_based_pick(intent: dict) -> str:
    """Fast keyword-based palette selection — no LLM call needed."""
    hint = " ".join([
    ...
    if any(k in hint for k in ["brutal", "bold", "yellow", "pop", "edgy", "loud"]):
        return "P-2"
    if any(k in hint for k in ["minimal", "editorial", "clean", "type", "read", "white"]):
        return "P-5"
    return "P-1"
...
{{
  ...
  "font_pair":        "Inter|Geist|Plus Jakarta Sans"
    ...
    # Build Tailwind design token set
    primary_base = p["primary"].split("-")[0]   # "blue"
    accent_base  = p["accent"].split("-")[0]    # "indigo"

    tailwind = {
        "container":      "max-w-7xl mx-auto px-6",
        "section":        "py-16 md:py-24",
        "card":           f"bg-{p['secondary']} {border_radius} {shadow_style} p-6",
        "btn_primary":    f"bg-{p['primary']} hover:opacity-90 text-white px-6 py-3 {border_radius} font-semibold transition-all",
        "btn_secondary":  f"border border-{primary_base}-400/30 text-{primary_base}-400 px-6 py-3 {border_radius} hover:bg-{primary_base}-400/10 transition-all",
