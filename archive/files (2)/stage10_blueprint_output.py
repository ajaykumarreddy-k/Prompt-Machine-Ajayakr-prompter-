...
def render_blueprint(blueprint: dict, verbose: bool = False) -> str:
    ...
    hier     = blueprint["hierarchy"]
    comps    = blueprint["components"]
    design   = blueprint["design"]
    skeleton = blueprint["skeletons"]
    ...
    for page in layout["pages"]:
        ...
        name = _c(C.B, page["name"])
        typ  = _c(C.GY, f"({page['type']})")
    ...
    for pname, pcomps in comps["component_map"].items():
        page_obj = next((pg for pg in layout["pages"] if pg["name"] == pname), {})
...
def save_blueprint(blueprint: dict) -> None:
    ...
    (out / "blueprint.json").write_text(
        json.dumps({
            ...
            "design":   {k: v for k, v in blueprint["design"].items() if k != "palette"},
        }, indent=2)
    )

    # prompt.md
    (out / "prompt.md").write_text(
        f"# {blueprint['intent']['product_name']} — Frontend Prompt\n\n{blueprint['prompt']}"
    )

    # skeletons
    for page_name, skel in blueprint["skeletons"].items():
